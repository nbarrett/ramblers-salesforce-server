# TypeScript implementation (Express / Fly.io)

The original production server for the Ramblers Salesforce API - the `typescript/` half of [`ramblers-salesforce-server`](../), sibling to the [Python implementation](../python). Wire format comes from [`@ramblers/sf-contract`](https://github.com/nbarrett/ramblers-salesforce-contract) v0.4.0; the two implementations are byte-identical on the wire. Deploys to Fly.io.
...
This implementation is production-only by design - it reads from Salesforce and carries no sample data of its own. To see the API work with synthetic data, run the [mock](https://github.com/nbarrett/ramblers-salesforce-mock) or the [Python server's demo mode](../python), which keeps demo data the mock's job and this server's job purely production.

## One repo of three

| Repo | Role | Live at |
|---|---|---|
| [`ramblers-salesforce-contract`](https://github.com/nbarrett/ramblers-salesforce-contract) | Shared wire-format package — types, Zod schemas, OpenAPI builder, error envelope, columns, port. | git tag `v0.4.0` |
...
| **`ramblers-salesforce-server`** (this repo) | Two Salesforce-backed server implementations - `typescript/` (this one, on Fly) and `python/` (live at [salesforce-server.ngx-ramblers.org.uk](https://salesforce-server.ngx-ramblers.org.uk)). | see python/ |
